...
class CVUSA(torch.utils.data.Dataset):
    def __init__(self, mode = '', root = '/home/parthpk/CVUSA/', same_area=True, print_bool=False, polar = '',args=None): #CV-dataset
        ...
        splits_path = '/home/ak119590/spring23/cap5610/semi_transgeo/'
        self.lbl_train_list = splits_path + 'splits/train-19zl_20%_1.csv'
        self.unbl_train_list = splits_path + 'splits/train-19zl_80%_1.csv'
        self.test_list = splits_path + 'splits/val-19zl.csv'
        ...
        self.lbl_id_idx_list = []
        self.unlbl_id_idx_list = []
        
        with open(self.lbl_train_list, 'r') as file:
            lbl_idx = 0
            for line in file:
                data = line.split(',')
                pano_id = (data[0].split('/')[-1]).split('.')[0]
                # satellite filename, streetview filename, pano_id
                self.lbl_id_list.append([data[0], data[1], pano_id])
                self.lbl_id_idx_list.append(lbl_idx)
                lbl_idx += 1
        with open(self.unlbl_train_list, 'r') as file:
            unlbl_idx = 0
            for line in file:
                data = line.split(',')
                pano_id = (data[0].split('/')[-1]).split('.')[0]
                # satellite filename, streetview filename, pano_id
                self.unlbl_id_list.append([data[0], data[1], pano_id])
                self.unlbl_id_idx_list.append(unlbl_idx)
                unlblidx += 1
        self.data_size = len(self.lbl_id_list) + len(self.unlbl_id_idx_list)
        if print_bool:
    ...
    def __len__(self):
        if self.mode == 'train':
            return len(self.lbl_id_idx_list + self.unlbl_id_idx_list)
        elif 'scan_val' in self.mode:
